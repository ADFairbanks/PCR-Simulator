...
function Mass_Action_Kinetics_ODE_Solve(y0, end_t, func, rates, dt = 0.05, t0 = 0, epsilon = 1e-30){
    ...
    for (var i=0; i<decoupled_rates.length; i++){
        ...
        for (var j=0; j<func_ret.length; j++){
            if (func_ret[j] < 0){ //Should this be <1?
...
function Sample_ODE_Func(y,t,rates_obj){
    ...
    var state_vector = new Array(S_COUNT).fill(0);
...
function demo(){
    ...
    var y0 = [100,0,0,0];
    t = 10;
    y = Mass_Action_Kinetics_ODE_Solve(y0,t,Sample_ODE_Func,rate_obj);
    document.getElementById("outdiv").innerHTML = "<p>Final state:"+y+"</p>";
}